...
internal static class Bip39
{
    ...
    public static byte[] GetEntropy(IReadOnlyCollection<string> words)
    {
        if (words == null)
        ...
        {
            var index = WordList.BinarySearch(word);
        ...
        var checksumBitsCnt = concatBitsCnt / 33;
        var entropyBitsCnt = concatBitsCnt - checksumBitsCnt;

        var entropy = new byte[entropyBitsCnt / 8];
        ...
        for (var i = 0; i < checksumBitsCnt; ++i)
    ...
    private static byte[] BytesToBits(IReadOnlyList<byte> data)
    {
        var bits = new byte[data.Count * 8];

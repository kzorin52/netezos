...
public class HDPath : IEnumerable<uint>
{
    ...
    /// <returns>
    ///     An <see cref="T:System.Collections.IEnumerator" /> object that can be used to iterate through the HDPath
    ///     indexes collection.
    /// </returns>
    public IEnumerator<uint> GetEnumerator()
    {
        return ((IEnumerable<uint>)Indexes).GetEnumerator();
    }
    ...
    private static bool TryParseIndex(string str, out uint ind)
    {
        if (str.Length == 0)
        {
            ind = 0;
    ...
    private static uint ParseIndex(string str)
    {
        if (str.Length == 0)
    ...
    private static string IndexToString(uint ind)
    {
